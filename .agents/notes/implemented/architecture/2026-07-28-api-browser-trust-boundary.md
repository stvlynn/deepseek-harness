...
Two boundaries stay deliberately out of scope of this fence: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and operator authentication is a separate capability seam ([GitHub account seam](2026-08-15-github-account-seam.md)) — the fence remains a confused-deputy defense, not that seam. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
