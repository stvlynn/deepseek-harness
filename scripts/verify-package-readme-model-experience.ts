...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-account': { kind: 'none', reason: 'Browser-side GitHub sign-in and account menu; registers nothing model-facing.' },
  ...
  'packages/identity/account': { kind: 'none', reason: 'The account seam authenticates the browser operator and never enters a model request, prompt, or tool schema.' },
  'packages/identity/account-better-auth': { kind: 'none', reason: 'The GitHub OAuth provider authenticates the browser operator and never enters a model request, prompt, or tool schema.' },
