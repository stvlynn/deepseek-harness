...
Seams are why one provider swap changes the whole product. Filesystem and subprocess providers share one execution world, so pointing them at a remote sandbox moves Bash, PTY, and LSP with them, with no provider forks. [Subagent providers](subsystems/subagent.md) vary just as widely behind one interface, from a fresh child agent to a delegated turn in another product. The Web account seam (`ctx.account`) authenticates browser operators with GitHub OAuth and isolates conversation lists by owner; ACP, SDK, and stdio stay process-owned.
...
| Authenticate Web operators | compose `ctx.account`; GitHub OAuth is the shipped provider |
