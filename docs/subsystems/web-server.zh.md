...
`host` 只接受 `127.0.0.1`（默认姿态）和 `0.0.0.0`（刻意的网络暴露）。webserver 本身没有 TLS 或 origin 策略；Web 组合包在 `/api/auth` 挂载 GitHub 账号，并据此主体守卫 `/api`。缺少 GitHub 配置的非回环绑定会在账号提供方加载阶段失败。dist 位置是认领席位的前端插件的组装事实。
